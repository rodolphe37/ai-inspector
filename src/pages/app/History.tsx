...
export default function History() {
  const { items, filter, search, setFilter, setSearch } = useHistoryStore();
  ...
  const filtered = items.filter((item) => {
    ...
    const matchesFilter =
      ...
      (filter === 'signals' && (item.status === 'possible_signal' || item.status === 'signal_detected' || item.status === 'c2pa_found')) ||
    ...
    <PageTransition>
      <div className="p-4 sm:p-6 lg:p-8 pb-20 lg:pb-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-muted">All your past analyses in one place.</p>
        </div>
          ...
          <div className="flex gap-1 p-1 bg-surface-2 rounded-lg w-fit">
            {filters.map((f) => (
              ...
              >
                {f.label}
                {filter === f.key && (
                  <motion.div layoutId="filter-indicator" className="absolute inset-0 bg-surface rounded-md -z-10" />
                )}
              </button>
            ))}
          </div>
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-3.5">
                          <Link to={`/app/results/${analysis.id}`} className="text-sm font-medium hover:text-primary">
                            {analysis.name}
                          </Link>
                        </td>
                        ...
                        <td className="px-5 py-3.5 hidden md:table-cell">
                          <span className="text-sm tabular-nums">{analysis.score}%</span>
                        </td>
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button className="p-1.5 rounded-lg hover:bg-surface-2 text-muted hover:text-error transition-colors">
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={page === 1}
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-default text-sm hover:bg-surface-2 disabled:opacity-50 transition-colors"
                  >
